...
--				 ((0.00033 * (power(size1,3.175))) * number_male_size1)
    WHEN mid_latitude > 58.65 and a.cruise >= 197501
			THEN
				 ((0.000502 * (power(size1,3.107158))) * number_male_size1)
		ELSE 0
		END)  wgt_male_size1
...
create table smbkc_cpuenum_sizegroup as select * from bk_cpuenum_sizegroup
where gis_station in (select station_id from strata_bkc_newtimeseries where district like 'St. Matthew%');
...
--Calculated weighted temperature

drop table smbkc_cpuenum_weighted_temp;

create table smbkc_cpuenum_weighted_temp as 
...
select * from SMBKC_CPUENUM_WEIGHTED_TEMP;