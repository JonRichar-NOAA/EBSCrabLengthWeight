...
--NEWSHELL formula for a power(10,(-3.38775+ (0.00216*c.male_weighted_gear_temp))
--OLDSHELL formula for a: power(10,(-3.38775 + 0.02593 + (0.00216*c.male_weighted_gear_temp)+(0.00466*c.male_weighted_gear_temp*1))
...
(CASE
    ...
    ELSE 0
    END) wgt_male_size1
from rk_number_size1_male a, haul_newtimeseries_noretow b,rkc_male_sw_temp_params p
where species_code = 69322 
and a.cruise = p.cruise
and b.cruise = p.cruise
...
(CASE
 ...
 --     THEN ((0.01027 * (power(size1,2.38849))) * number_female_size1)
    ...
    WHEN a.cruise >= 197501 and clutch_size > 1
      THEN ((p.log10_a * (power(size1,p.b))) * number_female_size1)
    ELSE 0
    END) wgt_female_size1
from rk_number_size1_female a, haul_newtimeseries_noretow b, rkc_matfemale_sw_temp_params p
where species_code = 69322 
and a.hauljoin(+) = b.hauljoin
and a.cruise = p.cruise
order by a.cruise,a.vessel,a.haul,a.gis_station,size1;
...
drop table rk_bb_leg1_biomass_new_model_ML;

create table rk_bb_leg1_biomass_new_model_ML as
select a.survey_year,
...
/*
drop table rk_bb_leg1_abundance_new_model_ML;

create table rk_bb_leg1_abundance_new_model_ML as
select a.survey_year,
