...
Male_EBSCO_weights_and_temps <- read_csv("//akc0ks-n220/KOD_Research/RKC-BKC/Survey L-W paper- Jon/Opilio/Male_EBSCO_weights_temps_analysis.csv")
...
LW8=function(a,aTr,b,bTr,sd){
  a=a+aTr*(MEBSCO_mat$Temperature>3.5)
  b=b+bTr*(MEBSCO_mat$Temperature>3.5)
  ave=log(MEBSCO_mat$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO_mat$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
...
LW13=function(a,aOS,aTr,b,bTr,sd){
  a=a+aTr*(MEBSCO_mat$Temperature>3.5)+aOS*(MEBSCO_mat$SHELL_CONDITION!=2)
  b=b+bTr*(MEBSCO_mat$Temperature>3.5)
  ave=log(MEBSCO_mat$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO_mat$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
...
LW14=function(a,aTr,b,bOS,bTr,sd){
  a=a+aTr*(MEBSCO_mat$Temperature>3.5)
  b=b+bTr*(MEBSCO_mat$Temperature>3.5)+bOS*(MEBSCO_mat$SHELL_CONDITION!=2)
  ave=log(MEBSCO_mat$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO_mat$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
...
LW23=function(a,aTr,a2Tr,aOS,b,sd){
  a=a+aTr*(MEBSCO_mat$Temperature>3.5)+a2Tr*(MEBSCO_mat$Temperature>3.5)*(MEBSCO_mat$SHELL_CONDITION==2)+aOS*(MEBSCO_mat$SHELL_CONDITION!=2)
  ave=log(MEBSCO_mat$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO_mat$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
