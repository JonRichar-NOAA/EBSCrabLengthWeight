...
FEBSCO%>% filter(CLUTCH_SIZE==4|CLUTCH_SIZE==5|CLUTCH_SIZE==6)->FEBSCO_mat_4_5_6
FEBSCO%>% filter(CLUTCH_SIZE>=1)->FEBSCO_mat
# column names
names(FEBSCO_mat)
min(FEBSCO_mat$Temperature_MATFEM)
max(FEBSCO_mat$Temperature_MATFEM)
...
LW1a=function(a,b,sd){
  ave=log10(FEBSCO_mat$WIDTH)*b+a
  -sum(dnorm(log10(FEBSCO_mat$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
param=list(a=-8,b=3,sd=0.1)
