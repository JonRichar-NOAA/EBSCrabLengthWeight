...
LW8=function(a,aTr,b,bTr,sd){
  a=a+aTr*(MEBSCO$Temperature>0.0)
  b=b+bTr*(MEBSCO$Temperature>0.0)
  ave=log(MEBSCO$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
...
LW13=function(a,aOS,aTr,b,bTr,sd){
  a=a+aTr*(MEBSCO$Temperature>0.0)+aOS*(MEBSCO$SHELL_CONDITION!=2)
  b=b+bTr*(MEBSCO$Temperature>0.0)
  ave=log(MEBSCO$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
...
LW14=function(a,aTr,b,bOS,bTr,sd){
  a=a+aTr*(MEBSCO$Temperature>0.0)
  b=b+bTr*(MEBSCO$Temperature>0.0)+bOS*(MEBSCO$SHELL_CONDITION!=2)
  ave=log(MEBSCO$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
...
LW23=function(a,aTr,a2Tr,aOS,b,sd){
  a=a+aTr*(MEBSCO$Temperature>0.0)+a2Tr*(MEBSCO$Temperature>0.0)*(MEBSCO$SHELL_CONDITION==2)+aOS*(MEBSCO$SHELL_CONDITION!=2)
  ave=log(MEBSCO$WIDTH)*b+a
  -sum(dnorm(log(MEBSCO$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
