...
names(FBBRKC)
min(FBBRKC$Temperature_female)
max(FBBRKC$Temperature_female)
...
LW1a=function(a,b,sd){
  ave=log10(FBBRKC_mat$LENGTH)*b+a
  -sum(dnorm(log10(FBBRKC_mat$WEIGHT), mean=ave,sd=sd,log=TRUE))
}
param=list(a=-8,b=3,sd=0.1)
