...
# 5/4/2021
# Calculate width/weight regression models for EBSCO females by clutch-size AND compare across shell condition categories
library(tidyverse)
library(stats)
library(nlme)
library(ggpubr)
...
############################ Fit followup model ##########################################
fit4<-lm(log.weight~log.width,data=os_egg4females_analysis)
summary(fit4)
coef(fit4)
cf4<-as.matrix(coef(fit4))

plot(log.weight~log.width,data=os_egg4females_analysis)
...
ggplot(analysis_matfemales, aes(x = log.width, y = log.weight, group = SC)) +
  geom_point(aes(colour = factor(SC)))+
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO(CS4, New shell and old shell) log-transformed")
...
ggplot(analysis_matfemales, aes(x = log.width, y = log.weight, group = SC,color = SC,shape=SC)) +
  ...
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO(New shell and old shell, CS4, log-transformed)")
...
fit5<-lm(log.weight~log.width,data=ns_female)
summary(fit5)
coef(fit5)
############################## check diagnostics #################################################
dev.new()
...
############################ Fit followup model ##########################################
fit6<-lm(log.weight~log.width,data=ns_egg5females_analysis)
summary(fit6)
coef(fit6)
...
fit7<-lm(log.weight~log.width,data=os_female)
summary(fit7)
coef(fit7)
...
############################ Fit followup model ##########################################
fit8<-lm(log.weight~log.width,data=os_egg5females_analysis)
summary(fit8)
coef(fit8)
cf8<-as.matrix(coef(fit8))

##### Compare new shell and old shell CS5 crab #########
plot(log.weight~log.width,data=os_egg5females_analysis)
...
ggplot(analysis_matfemales, aes(x = log.width, y = log.weight, group = SC,color = SC,shape=SC)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO (New shell and old shell, CS5, log-transformed)")
...
fit9<-lm(log.weight~log.width,data=ns_female)
summary(fit9)
coef(fit9)
############################## check diagnostics #################################################
dev.new()
...
############################ Fit followup model ##########################################
fit10<-lm(log.weight~log.width,data=ns_egg6females_analysis)
summary(fit10)
coef(fit10)
...
fit11<-lm(log.weight~log.width,data=os_female)
summary(fit11)
coef(fit11)
...
plot(log.weight~log.width,data=os_egg6females_analysis)

abline(a=cf10[1,1],b=cf10[2,1])
abline(a=cf12[1,1],b=cf12[2,1])

exp(cf12[1,1])
# exp(W) = -3.203268 + 2.827141 * log10(L) on transformed scale       #updated for females
...
mod1<-aov(log.weight~log.width*SC,data = analysis_matfemales)
summary(mod1)									# p-value = 0.488 on interaction term. No evidence slopes are different

mod2<-aov(log.weight~log.width+SC,data = analysis_matfemales)
summary(mod2)									# p-value for SC = 1.13e-10. Intercepts are significantly different
