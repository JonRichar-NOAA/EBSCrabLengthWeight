...
exp(cf2[1,1])
# log(W) = -7.168785 + 2.801220 * log10(W) on transformed scale       #updated for females
# W = exp(-7.168785) * L^(2.801220)  on original scale                #updated for females
...
cf2[2,1]
A
# a = 0.0007718846      #updated for females
# b = 2.801220          #updated for females
##########################################################################################################################
############################ Clutch size 5 ###############################################################################
##########################################################################################################################
...
# log10(W) = -7.111529 + 2.790909  * log10(L) on transformed scale       #updated for females
# W = exp(-7.111529) * L^(2.790909)  on original scale                #updated for females
...
cf4[2,1]
A
# a = 0.000816958      #updated for females
# b = 2.790909         #updated for females
...
exp(cf6[1,1])
# log(W) = -7.473500 + 2.882551  * log10(L) on transformed scale       #updated for females
# W = exp(-7.473500) * L^(2.882551)  on original scale              #updated for females
...
int<-cf6[1,1]
A<-(exp(int)*exp(v6/2))
A                         #0.0005692038                              #updated for females
...
ebsco_sc_points<-ggplot(analysis_matfemales, aes(x = log.width, y = log.weight, group = ClutchSize)) +
  geom_point(aes(colour = factor(ClutchSize)))+
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO(Clutch 5 and 6) log-transformed")
...
ebsco_sc_lines<-ggplot(analysis_matfemales, aes(x = log.width, y = log.weight, group = ClutchSize,color = ClutchSize,shape=ClutchSize)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO(Clutch 5 and 6, log-transformed)")
...
exp(cf8[1,1])
# log10(W) =-7.567123 + 2.91246  * log10(L) on transformed scale       #updated for females
# W = exp(-7.567123) * L^(2.91246)  on original scale                #updated for females
