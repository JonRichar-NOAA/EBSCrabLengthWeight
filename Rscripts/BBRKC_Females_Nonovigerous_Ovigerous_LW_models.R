...
############################ log transformed ##################################################
dev.new()
p<-ggplot(ns_nonovigfemales_analysis, aes(x = log.length, y = log.weight)) +
  geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="New shell (SC2) nonovigerous females-log transformed, influential points removed")

############################ Fit followup model ##########################################
fit.im<-lm(log.weight~log.length,data=ns_nonovigfemales_analysis)
summary(fit.im)
coef(fit.im)
...
exp(cf2[1,1])
# log(W) = -7.582706  + 3.07793 * log(L) on transformed scale       #updated for females
...
############################ log transformed ##################################################
dev.new()
p<-ggplot(ns_ovigfemales_analysis, aes(x = log.length, y = log.weight)) +
      geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="New shell (SC2) females-log transformed, influential points removed")

############################ Fit followup model ##########################################
fit2<-lm(log.weight~log.length,data=ns_ovigfemales_analysis)
summary(fit2)
coef(fit2)
...
exp(cf2[1,1])
# log(W) = -5.37042 + 2.608199 * log(L) on transformed scale           #Updated for females
