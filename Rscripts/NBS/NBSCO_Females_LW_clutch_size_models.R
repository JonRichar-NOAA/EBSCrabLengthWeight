...
# log(W) = -7.468750 + 2.799047  * log(L) on transformed scale       #updated for females
...
exp(cf4[1,1])
# log(W) = -7.031261 + 2.786251 * log(L) on transformed scale       #updated for females
...
ns_imfemale<-as.data.frame(cbind(alleggfemales,YEAR,log.width,log.weight))   		 # Bind new data objects and crab data in data frame  
ns_imfemale                    							  		 # inspect data
names(ns_imfemale)											 # Check column names
...
############################ log transformed ##################################################
dev.new()
p<-ggplot(alleggfemales_analysis, aes(x = log.width, y = log.weight)) +
  geom_point()
p+ labs(x="ln(width)",y="ln(weight)", title="All egg bearing females-log transformed, influential points removed")

############################ Fit followup model ##########################################
fit10<-lm(log.weight~log.width,data=alleggfemales_analysis)
summary(fit10)
coef(fit10)
