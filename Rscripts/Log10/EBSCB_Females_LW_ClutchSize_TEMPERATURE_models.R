...
dev.new()
p<-ggplot(ns_noeggfemales, aes(x = WIDTH, y = WEIGHT)) +
  geom_point()
...
#View(temp_ns_eggfemales)

mod1<-aov(log10.weight~log10.width*TEMP,data = temp_ns_eggfemales)							# p = 0.193 on interaction term = significant interaction--suggests different slopes
summary(mod1)									

mod2<-aov(log10.weight~log10.width+TEMP,data = temp_ns_eggfemales)							# p = 00.0291 for TEMP: temperature-based regression lines have different slopes
summary(mod2)									

anova(mod1,mod2)														# p = 0.1931...removing interaction term significantly affects model								
...
ebscb_temp_points<-ggplot(temp_ns_eggfemales, aes(x = log10.width, y = log10.weight, group = TEMP)) +
...
ebscb_temp_lines <- ggplot(temp_ns_eggfemales, aes(x = log10.width, y = log10.weight, group = TEMP,color = TEMP,shape=TEMP)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male EBS CB (New shell,log-transformed)")
...
mod1<-aov(log10.weight~log10.width*TEMP,data = temp_ns_eggfemales_2)							# p = 0.840 on interaction term = significant interaction--suggests different slopes
summary(mod1)									

mod2<-aov(log10.weight~log10.width+TEMP,data = temp_ns_eggfemales_2)							# p = 0.852 for TEMP: temperature-based regression lines have different slopes
summary(mod2)									
...
ebscb_temp_points<-ggplot(temp_ns_eggfemales_2, aes(x = log10.width, y = log10.weight, group = TEMP)) +
  geom_point(aes(colour = TEMP))+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Female EBS CB (New shell,log-transformed)")
...
ebscb_temp_lines <- ggplot(temp_ns_eggfemales_2, aes(x = log10.width, y = log10.weight, group = TEMP,color = TEMP,shape=TEMP)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Female EBS CB (New shell,log-transformed)")
...
# between regression lines.
# Cold years (from tech memo, years with retows):2000, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2017
# Warm years (from tech memo, years without retow):  2013, 2014, 2015, 2016, 2018, 2019
...
ebscb_temp_points<-ggplot(temp_ns_noeggfemales, aes(x = log10.width, y = log10.weight, group = TEMP)) +
  geom_point(aes(colour = TEMP))+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male EBS CB (New shell,log-transformed)")
...
ebscb_temp_lines <- ggplot(temp_ns_noeggfemales, aes(x = log10.width, y = log10.weight, group = TEMP,color = TEMP,shape=TEMP)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male EBS CB (New shell,log-transformed)")
