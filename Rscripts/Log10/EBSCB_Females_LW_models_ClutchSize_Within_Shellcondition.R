...
ebscb_sc_points<-ggplot(analysis_matfemales, aes(x = log10.width, y = log10.weight, group = ClutchSize)) +
  geom_point(aes(colour = factor(ClutchSize)))+
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CB(Clutch 5 and 6) log-transformed")
...
ebscb_sc_lines<-ggplot(analysis_matfemales, aes(x = log10.width, y = log10.weight, group = ClutchSize,color = ClutchSize,shape=ClutchSize)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CB(Clutch 5 and 6, log-transformed)")
...
summary(mod2)									# p-value for CS = 3.7e-10. Intercepts are significantly different, 1.81e-12 with CS4
...
# log10(W) = -3.200837  + 2.825224   * log10(L) on transformed scale       #updated for females
# W = exp(-3.200837) * L^(2.825224 )  on original scale                #updated for females
...
# a = 0.0006301015    #updated for females
# b = 2.825224        #updated for females
