...
co_bias<-read.csv("co_biomass_newmodel_biascorrected.csv")
...
ggplot(bkc_bias,aes(x=bk_year))+
  ...
  labs(color = "Category",x = "Survey Year", y = "Percent difference",title="St Matthew blue king crab")
...
ggarrange(rkc_bias, e166_bias,w166_bias, ebs_co_bias,
          labels = c("a.)", "b.)", "c.)", "d.)"),
          ncol = 2, nrow = 2)
