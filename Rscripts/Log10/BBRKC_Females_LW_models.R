...
p<-ggplot(ns_imfemale, aes(x = log10.length, y = log10.weight)) +
  geom_point()
p+ labs(x="Log10(length)",y="Log10(weight)", title="New shell (SC2) immature females-log transformed")
...
p<-ggplot(ns_immatfemales_analysis, aes(x = log10.length, y = log10.weight)) +
  geom_point()
p+ labs(x="Log10(length)",y="Log10(weight)", title="New shell (SC2) immature females-log transformed, influential points removed")
...
p<-ggplot(ns_female, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="Log10(length)",y="Log10(weight)", title="New shell (SC2) females-log transformed")
...
p<-ggplot(ns_matfemales_analysis, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="Log10(length)",y="Log10(weight)", title="New shell (SC2) females-log transformed, influential points removed")
...
p<-ggplot(os_female, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="Log10(length)",y="Log10(weight)", title="Old shell (SC3+SC4) females-log transformed")
...
p<-ggplot(os_matfemales_analysis, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="Log10(length)",y="Log10(weight)", title="Old shell (SC3+SC4) BBRKC females-log transformed, influential points removed")
...
ggplot(analysis_females, aes(x = log10.length, y = log10.weight, group = SC)) +
     geom_point(aes(colour = factor(SC)))+
	labs(x="Log10(Length)",y="Log10(Weight)", title="Female BBRKC (New shell and old shell, log-transformed)")
...
summary(reg_mod)
######################## Rerun using car ANova command and type III errors (more appropriate for ANCOVA)###############
...
mod1<-aov(log10.weight~log10.length*TEMP,data = temp_ns_females)							# p = 0.114 on interaction term = no interaction--suggests no difference in slopes
summary(mod1)									

mod2<-aov(log10.weight~log10.length+TEMP,data = temp_ns_females)							# p  = 0.106 for TEMP: temperature-based regression lines do not have different slopes
summary(mod2)									

anova(mod1,mod2)														# p = 0.11450.0001924...removing interaction term does not affect model								
...
ggplot(temp_ns_females, aes(x = log10.length, y = log10.weight, group = TEMP)) +
  ...
  labs(x="Log10(Length)",y="Log10(Weight)", title="Mature female BBRKC (New shell,log-transformed)")
...
mod1<-aov(log10.weight~log10.length*TEMP,data = temp_ns_females)							# p = 0.000192 on interaction term = significant interaction--suggests different slopes
summary(mod1)									

mod2<-aov(log10.weight~log10.length+TEMP,data = temp_ns_females)							# p < 2e-16 for TEMP: temperature-based regression lines have different slopes
summary(mod2)									
