...
alleggfemales<-subset(ddf1,SEX==2 & CLUTCH_SIZE>1 & SHELL_CONDITION==2|SHELL_CONDITION==3|SHELL_CONDITION==4)
...
exp(cf2[1,1])
# log10(W) = -7.161370 + 2.780558 * log10(L) on transformed scale       #updated for females
...
# a = 0.0008149472      #updated for females
# b = 2.768817          #updated for females
...
# log10(W) = -7.468750 + 2.799047  * log10(L) on transformed scale       #updated for females
# W = exp(-7.468750) * L^(2.799047)  on original scale                #updated for females
# a = 0.000570641                                                 #updated for females
# b = 2.875724                                                     #updated for females
##############################################################################################
######################## Apply bias-correction procedure #####################################
cf2
names(fit2)
...
# a = 0.0005719966    #updated for NBS females
# b = 2.875724         #updated for NBS females
...
exp(cf4[1,1])
# log10(W) = -7.031261 + 2.786251 * log10(L) on transformed scale       #updated for females
# W = exp(-7.031261) * L^(2.786251)  on original scale              #updated for females
# a = 0.0008838164                                                 #updated for females
# b = 2.786251                                                     #updated for females
...
int<-cf4[1,1]
A<-(exp(int)*exp(v4/2))
A                         #0.0008866768                             #updated for females
...
#####################################################################################################
############################ All egg-bearing females ########################################################
######################################################################################################
plot(alleggfemales$WEIGHT~alleggfemales$WIDTH)
############################## Plot base data in GG plot ############################################################
dev.new()
p<-ggplot(alleggfemales, aes(x = WIDTH, y = WEIGHT)) +
  geom_point()
p+ labs(title="Non-egg bearing females")

############################# Add fields for analysis ########################################################
Year <- substring(alleggfemales$CRUISE, 1,4) 

YEAR <- as.factor(Year)
log.width<-log(alleggfemales$WIDTH)
log.weight <- log(alleggfemales$WEIGHT)

log10.width<-log10(alleggfemales$WIDTH)
log10.weight <- log10(alleggfemales$WEIGHT)
ns_imfemale<-as.data.frame(cbind(alleggfemales,YEAR,log10.width,log10.weight))   		 # Bind new data objects and crab data in data frame  
ns_imfemale                    							  		 # inspect data
names(ns_imfemale)											 # Check column names
...
p<-ggplot(ns_imfemale, aes(x = log10.width, y = log10.weight)) +
  geom_point()
p+ labs(x="ln(width)",y="ln(weight)", title="All egg-bearing females-log transformed")
...
ns_imfemale$Cooks_D <- cooks.distance(fit1)
alleggfemales_analysis<-subset(ns_imfemale, Cooks_D < (4/(nrow(ns_imfemale))))

nrow(ns_imfemale)-nrow(alleggfemales_analysis)    #110 obs removed based on Cook's Distance
nrow(alleggfemales_analysis)

##################################################################################################
############################# Plot using editted dataset #########################################
dev.new()
p<-ggplot(alleggfemales_analysis, aes(x = WIDTH, y = WEIGHT)) +
  geom_point()
p+ labs(title="All ovigerous females (N infuential points removed")


############################ log transformed ##################################################
dev.new()
p<-ggplot(alleggfemales_analysis, aes(x = log10.width, y = log10.weight)) +
  geom_point()
p+ labs(x="ln(width)",y="ln(weight)", title="All egg bearing females-log transformed, influential points removed")

############################ Fit followup model ##########################################
fit10<-lm(log10.weight~log10.width,data=alleggfemales_analysis)
summary(fit10)
coef(fit10)
...
names(summary(fit10))
###############################################################################################
v2<-(summary(fit10)$sigma)**2  #Variance 
v2
int<-cf10[1,1]
A<-(exp(int)*exp(v2/2))
A                         # 0.0008149472
...
ebsCO_sc_points<-ggplot(analysis_matfemales, aes(x = log10.width, y = log10.weight, group = SC)) +
     geom_point(aes(colour = factor(SC)))+
	labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO(New shell and old shell) log-transformed")
...
ebsCO_sc_lines<-ggplot(analysis_matfemales, aes(x = log10.width, y = log10.weight, group = SC,color = SC,shape=SC)) +
	geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
	labs(x="Ln(Width)",y="Ln(Weight)", title="Female EBS CO(New shell and old shell, log-transformed)")
...
ebsCO_sc_points_sub <- ggplot(analysis_matfemales_sub, aes(x = log10.width, y = log10.weight, group = SC)) +
     geom_point(aes(colour = SC))+
...
ebsCO_sc_lines_sub <-ggplot(analysis_matfemales_sub, aes(x = log10.width, y = log10.weight, group = SC,color = SC,shape=SC)) +
	geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
