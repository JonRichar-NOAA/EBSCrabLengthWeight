...
p<-ggplot(ns_male, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="New shell (SC2) NBS RKC males-log transformed")
...
nrow(ns_male)-nrow(ns_males_analysis)    #8 obs removed based on Cook's Distance

nrow(ns_males_analysis)    #152
...
p<-ggplot(ns_males_analysis, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="New shell (SC2) NBS RKC males-log transformed, influential points removed")
...
p<-ggplot(os_male, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="Old shell (SC3+SC4) NBS RKC males-log transformed")
...
p<-ggplot(os_males_analysis, aes(x = log10.length, y = log10.weight)) +
      geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="Old shell (SC3+SC4) NBS RKC males-log transformed, influential points removed")
...
YEAR <- as.factor(Year)
log10.length<-log10(all_males$LENGTH)
log10.weight <- log10(all_males$WEIGHT)
all_male<-as.data.frame(cbind(all_males,YEAR,log10.length,log10.weight))   		 # Bind new data objects and crab data in data frame  
...
############################ Plot log transformeddata in GGplot #############################################
dev.new()
p<-ggplot(all_male, aes(x = log10.length, y = log10.weight)) +
  geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="All NBS RKC males-log transformed")


############################## Fit initial model ########################################################

fit1<-lm(log10.weight~log10.length,data=all_male)
summary(fit1)
coef(fit1)
############################## check diagnostics #################################################
dev.new()
...
p<-ggplot(all_males_analysis, aes(x = LENGTH, y = WEIGHT)) +
  geom_point()
p+ labs(title="All NBS RKC males-infuential points removed")


############################ log transformed ##################################################
dev.new()
p<-ggplot(all_males_analysis, aes(x = log10.length, y = log10.weight)) +
  geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="New shell (SC2) NBS RKC males-log transformed, influential points removed")

############################ Fit followup model ##########################################
fit8<-lm(log10.weight~log10.length,data=all_males_analysis)
summary(fit8)
coef(fit8)
...
q<-ggplot(analysis_males, aes(x = LENGTH, y = WEIGHT, group = SC)) +
     geom_point(aes(colour = factor(SC)))
q+ labs(x="length(mm)",y="Weight(g)", title="Male NBS RKC (New shell and old shell)")
...
ggplot(analysis_males, aes(x = log10.length, y = log10.weight, group = SC)) +
     geom_point(aes(colour = factor(SC)))+
	labs(x="Ln(length)",y="Ln(Weight)", title="Male NBS RKC (New shell and old shell, log-transformed)")
...
mod1<-aov(log10.weight~log10.length*SC,data = analysis_males)			# p = 0.107 -- slopes are bnot significantly different
summary(mod1)

mod2<-aov(log10.weight~log10.length+SC,data = analysis_males)			# p = 9.33e-08: intercepts are significantly different
summary(mod2)

anova(mod1,mod2)										# p = 0.1071--interaction term is not significant to model
