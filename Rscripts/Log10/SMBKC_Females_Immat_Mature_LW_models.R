...
exp(cf2[1,1])
# log10(W) = -8.402001  + 3.272900  * log10(L) on transformed scale       #updated for females
...
fit1a<-lm(log10.weight~log10.length,data=all_female)
summary(fit1a)
coef(fit1a)
############################## check diagnostics #################################################
dev.new()
...
############################ log transformed ##################################################
dev.new()
p<-ggplot(all_matfemales_analysis, aes(x = log10.length, y = log10.weight)) +
  geom_point()
p+ labs(x="ln(length)",y="ln(weight)", title="Mature females-log transformed, influential points removed")

############################ Fit followup model ##########################################
fit2a<-lm(log10.weight~log10.length,data=all_matfemales_analysis)
summary(fit2a)
coef(fit2a)

cf2<-as.matrix(coef(fit2a))
exp(cf2[1,1])
# log10(W) = -8.670425 + 3.341792 * log10(L) on transformed scale           #Updated for females
...
############################# points only ############################################################################
ggplot(analysis_males, aes(x = log10.length, y = log10.weight, group = Mat)) +
  geom_point(aes(colour = factor(Mat)))+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male BBRKC (New shell and old shell, log-transformed)")

bbrkc_Mat_points <- ggplot(analysis_males, aes(x = log10.length, y = log10.weight, group = Mat)) +
  geom_point(aes(colour = Mat))+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male BBRKC (New shell and old shell, log-transformed)")

############################# lines ONLY ###############################################################################
ggplot(analysis_males, aes(x = log10.length, y = log10.weight, group = Mat,color = Mat,shape=Mat)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male BBRKC(New shell and old shell, log-transformed)")


bbrkc_Mat_lines <-ggplot(analysis_males, aes(x = log10.length, y = log10.weight, group = Mat,color = Mat,shape=Mat)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male BBRKC(New shell and old shell, log-transformed)")
...
ggscatter(analysis_males, x="log10.length",y="log10.weight",color="Mat", add="reg.line"
)+
  stat_regline_equation(
    aes(label=paste(..eq.label.., ..rr.label.., sep="~~~~"), color = factor(Mat))
  )
##################### Alternative 2 ####################################################
p<-ggplot(analysis_males, aes(x = log10.length, y = log10.weight), group = Mat,shape=Mat) +
  geom_point(aes(colour = Mat))
