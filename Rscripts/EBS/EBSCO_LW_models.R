...
write.csv(analysis_males,"EBS_CO_Analysis_males.csv")
...
mod1<-aov(log.weight~log.width*SC,data = analysis_males)			# p = 1.94e-15--significant interaction -- slopes different
summary(mod1)

mod2<-aov(log.weight~log.width+SC,data = analysis_males)			# p< 2e-16: interceptes are significantly different
summary(mod2)
...
ebsco_temp_points<-ggplot(temp_ns_males, aes(x = log.width, y = log.weight, group = TEMP)) +
...
ebsco_temp_lines <- ggplot(temp_ns_males, aes(x = log.width, y = log.weight, group = TEMP,color = TEMP,shape=TEMP)) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
  labs(x="Ln(Length)",y="Ln(Weight)", title="Male EBS CO (New shell,log-transformed)")
...
ebsco_sc_points_sub <- ggplot(analysis_males_sub, aes(x = log.width, y = log.weight, group = SC)) +
     geom_point(aes(colour = SC))+
...
ebsco_sc_lines_sub <-ggplot(analysis_males_sub, aes(x = log.width, y = log.weight, group = SC,color = SC,shape=SC)) +
	geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
	labs(x="Ln(Width)",y="Ln(Weight)", title="Male EBS CO(New shell and old shell,log transformed and truncated)")
...
ebsco_sc_points_sub2 <- ggplot(analysis_males_sub2, aes(x = log.width, y = log.weight, group = SC)) +
     geom_point(aes(colour = SC))+
...
ebsco_sc_lines_sub2 <-ggplot(analysis_males_sub2, aes(x = log.width, y = log.weight, group = SC,color = SC,shape=SC)) +
	geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+
	labs(x="Ln(Width)",y="Ln(Weight)", title="Male EBS CO(New shell and old shell, truncated at 76 mm)")
...
########################################################################################################################
############################### Run ANCOVA analyses to determine if base and series 2 are statistically different ############################
